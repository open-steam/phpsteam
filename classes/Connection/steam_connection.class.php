...
class steam_connection {
	...
	public function login($pLogin, $pPassword, $relogin = false) {
		...
		*/
		if ($this->login_status) {
			...
			if (!$relogin) {
				$this->login_arguments = $result->get_arguments();
			}

			if (!$relogin) {
				$this->server_version = $this->login_arguments[1];
			}

			if (!$relogin) {
				$this->last_reboot = $this->login_arguments[2];
			}

			if (!$relogin) {
				$this->pike_version = $this->login_arguments[4];
			}

			if (!$relogin) {
				$this->database = $this->login_arguments[5];
			}

			if (!$relogin) {
				$this->root_room = $this->login_arguments[6];
			}

			if (!$relogin) {
				$this->steam_group = $this->login_arguments[7];
			}

			if (!$relogin) {
				$this->server_config = $this->login_arguments[10];
			}
	...
	public function send_command(array $pCommandBuffer) {
		...
		foreach ($pCommandBuffer as $request) {
			...
			$bytes = @fwrite($this->socket, $encoded_request, strlen($encoded_request));
			if (!$bytes) {
	...
	public function buffer_attributes_request($pObject, $pAttributes, $pSourceObjectID = 0) {
		...
		$transaction_id = $this->predefined_command($object, "query_attributes", array($pAttributes), 1);
	...
	public function get_module($pModule) {
		if (!isset($this->login_arguments[8][$pModule])) {
			return 0;
		}
	...
	public function get_factory($pType) {
		if (!isset($this->login_arguments[9][$pType])) {
			return 0;
		}
	...
	public function exception($pCode, $pDetails = "", $allow_backtrace = true) {
		...
		switch ($pCode) {
			case 100:
				$e = new steam_exception($this->get_login_user_name(), "Server unknown: " . $pDetails, 100, $allow_backtrace);
				break;

			case 110:
				$e = new steam_exception($this->get_login_user_name(), "Could not connect to server: " . $pDetails, 110, $allow_backtrace);
				break;
			...
			case 120:
				...
				$e = new steam_exception($this->get_login_user_name(), $msg, 120, $allow_backtrace);
				break;

			case 150:
				$e = new steam_exception($this->get_login_user_name(), "Could not reconnect", 150, $allow_backtrace);
				break;

			case 200:
				$e = new steam_exception($this->get_login_user_name(), "Got no result back from steam: " . $pDetails, 200, $allow_backtrace);
				break;
	 ...
	 * @return
	 */
	public function get_config_value($pKey) {
