...
class steam_user extends steam_container {
	...
	public function add_unconfirmed_contact($contact_id, $steam_connector = null) {
		if (!isset($steam_connector)) {
			$steam_connector = $GLOBALS["STEAM"];
		}

		$unconfirmed_old = $this->get_attribute("USER_CONTACTS_TOCONFIRM");
		if (!is_array($unconfirmed_old)) {
			$unconfirmed_old = array();
		}
	...
	public function add_confirmed_contact($contact_id, $steam_connector = null) {
		...
		if (!is_array($confirmed_old)) {
			$confirmed_old = array();
		}

		if (!is_array($favourites_old)) {
			$favourites_old = array();
		}

		if (!is_array($to_confirm)) {
			$to_confirm = array();
		}
	 ...
	 * @return
	 */
	public function get_email_forwarding($pBuffer = 0) {
		$forward_module = $this->get_steam_connector()->get_module("forward");
	...
	public function get_mails($pIncludeFolders = FALSE, $pFolder = 0) {
		...
		if ($pIncludeFolders) {
			$mails = $mailbox->get_annotations();
		} else {
			$mails = $mailbox->get_annotations(CLASS_DOCUMENT);
		}
	 ...
	 */
	public function create_sent_mail_folder($folder_name = NULL, $pBuffer = 0) {
	 ...
	 */
	public function set_sent_mail_folder($mail_folder, $pBuffer = 0) {
	 ...
	 */
	public function is_storing_sent_mail($pBuffer = 0) {
	 ...
	 * @return Boolean TRUE if sent mails are stored for the user, FALSE if not
	 */
	public function set_is_storing_sent_mail($store, $pBuffer = 0) {
	...
	public function get_confirmed_contacts() {
		...
		if (is_array($user_favourites_obj)) {
			foreach ($user_favourites_obj as $ufo) {
				if ($ufo instanceof steam_user || $ufo instanceof steam_group) {
