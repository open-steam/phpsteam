...
class FileContentIdPersistence extends FilePersistence {
	...
	public static function init() {
		if (!FILE_PERSISTENCE_BASE_PATH || FILE_PERSISTENCE_BASE_PATH === "") {
	...
	private function putToFile(steam_document $document, $handler, $lastContentId, $version) {
		...
		//move old version
		if ($version && ($lastContentId !== 0)) {
			// if version created and not initial save
	...
	public function lowDeleteContentFile(steam_document $document) {
		...
		if (!is_string($contentFile)) {
			// no content file, document never saved
		...
		if (file_exists($contentFile)) {
			...
			LoggerHelper::getInstance()->getLogger()->warning("content file is missing (id: " . $document->get_id() . "; file: " . $contentFile . ")");
		}
